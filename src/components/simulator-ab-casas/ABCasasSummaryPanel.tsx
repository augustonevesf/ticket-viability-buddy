...
import { generateInsightsABCasas } from "@/utils/generateInsightsABCasas";

type Status = ABCasasResults["status"];
...
export const ABCasasSummaryPanel: React.FC<Props> = ({ results, inputs, onSave, onExportPDF, idViabilidade }) => {
  ...
  return (
    <div className="sticky top-6 space-y-4">
      <div className="flex gap-2">
        <button onClick={onSave} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition-colors">
          <Save className="w-4 h-4" />
          {idViabilidade ? "Atualizar" : "Salvar"}
        </button>
        <button onClick={onExportPDF} className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-muted text-foreground text-sm font-semibold hover:bg-muted/80 transition-colors">
          <FileDown className="w-4 h-4" />
          PDF
        </button>
        <button onClick={() => setShowInsights(!showInsights)} className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-warning/10 text-warning border border-warning/20 text-sm font-medium hover:bg-warning/20 transition-colors">
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
            <div className="bg-card rounded-2xl p-5 shadow-card border border-primary/20">
              ...
              <div className="space-y-3">
                {generateInsightsABCasas(inputs, results).map((insight, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }} className="flex gap-3 p-3 rounded-xl bg-muted/50">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
                          ...
                          insight.impact === "medio" ? "bg-warning/10 text-warning" : "bg-muted text-muted-foreground"
                        }`}>{insight.impact}</span>
                      </div>
                ...
                {generateInsightsABCasas(inputs, results).length === 0 && (
                  <p className="text-xs text-muted-foreground text-center py-4">✅ Operação bem configurada!</p>
                )}
              </div>
            </div>
          </motion.div>
      ...
      {inputs.cliente.nome && (
        <div className="bg-primary/5 border border-primary/20 rounded-2xl px-4 py-3">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-semibold">Casa</span>
            <span className="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
              {inputs.configuracao.cashless_ficha === "cashless" ? "Cashless" : "Ficha"}
            </span>
            {inputs.configuracao.antecipado_100 && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-warning/10 text-warning font-medium">Antecipado (sem receita antec.)</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="bg-card rounded-2xl p-6 shadow-card">
        <div className="flex items-start justify-between mb-6">
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className={`text-[10px] italic ${cfg.text} text-center max-w-[140px]`}>{cfg.frase}</p>
            <div className="w-2.5 h-24 bg-muted rounded-full overflow-hidden relative">
              <motion.div className={`absolute bottom-0 left-0 right-0 rounded-full ${cfg.barColor}`} animate={{ height: `${barPct}%` }} transition={{ duration: 0.4 }} />
            </div>
          </div>
        </div>
        <div className="border-t border-border pt-4 space-y-1">
          ...
          <div className="flex justify-between items-center py-1.5">
            ...
            <span className="text-sm font-medium tabular-nums text-foreground">{formatCurrency(results.minimo_garantido)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card rounded-2xl p-5 shadow-card">
        ...
        <div className="divide-y divide-border">
          <div className="pb-3">
            ...
            <MetricRow label="Débito V/M" value={formatCurrency(results.faturamento_debito_vm)} muted />
            <MetricRow label="Débito Outros" value={formatCurrency(results.faturamento_debito_outros)} muted />
            <MetricRow label="Pix" value={formatCurrency(results.faturamento_pix)} muted />
            <MetricRow label="Crédito V/M" value={formatCurrency(results.faturamento_credito_vm)} muted />
            <MetricRow label="Crédito Outros" value={formatCurrency(results.faturamento_credito_outros)} muted />
      ...
      <div className="bg-card rounded-2xl p-5 shadow-card">
        ...
        <div className="divide-y divide-border">
          <div className="pb-3">
            ...
            <MetricRow label="Débito V/M" value={formatCurrency(results.receita_debito_vm)} success />
            <MetricRow label="Débito Outros" value={formatCurrency(results.receita_debito_outros)} success />
            <MetricRow label="Pix" value={formatCurrency(results.receita_pix)} success />
            <MetricRow label="Crédito V/M" value={formatCurrency(results.receita_credito_vm)} success />
            <MetricRow label="Crédito Outros" value={formatCurrency(results.receita_credito_outros)} success />
          ...
          {results.total_receitas_setup > 0 && (
            <div className="py-3">
              ...
              <MetricRow label="(−) Custo Setup (35%)" value={formatCurrency(results.custo_setup_automatico)} muted />
            </div>
          )}
          <div className="pt-3">
      ...
      <div className="bg-card rounded-2xl p-5 shadow-card">
        ...
        <div className="divide-y divide-border">
          ...
          <div className="py-3">
            ...
            <MetricRow label="(−) Redutor PIX" value={formatCurrency(results.custo_pix_redutor)} muted />
            <MetricRow label="(−) Custo Máquinas" value={formatCurrency(results.custo_maquinas)} muted />
            <MetricRow label="Total Adquirência + Máq." value={formatCurrency(Math.abs(results.total_custos_adquirencia))} bold />
          </div>
          <div className="py-3">
      ...
      <div className="bg-card rounded-2xl p-5 shadow-card border-l-4 border-primary">
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between items-center py-2">
            <span className="text-sm font-semibold text-foreground">Mínimo Garantido</span>
            <span className="text-base font-bold tabular-nums text-primary">{formatCurrency(results.minimo_garantido)}</span>
          </div>
